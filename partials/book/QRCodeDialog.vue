<template>
	...
	>
		...
		<tamplate v-if="student">
			<div
				class="flex items-center justify-center bg-green-100 rounded-lg py-3 px-3 mb-4 shadow-sm"
			>
				<div class="flex flex-col items-center">
					<span
						class="text-md md:text-lg leading-5 text-center font-bold text-green-800"
					>
						{{ student?.book_title }}
					</span>
					...
					<p class="text-sm md:text-lg text-gray-700 mt-1">
						...
						<span class="font-semibold"
							>{{ student?.name }} - {{ `${student?.class}º ano` }}</span
						>
					</p>
				</div>
			</div>
